...
Public Class ClsSeries
    ...
    Sub New(ByVal pCod_Serie As String, ByVal pFLG_Tipo_Doc As String, ByVal pAno_Trabalho As String)
        Cod_Serie = pCod_Serie
        FLG_Tipo_Doc = pFLG_Tipo_Doc
        Ano_Trabalho = pAno_Trabalho
        Contador = 0
        FLG_Ativo = True
    End Sub

    Friend Shared Sub Init_Series()
        Dim mAno As String = Year(Date.Now).ToString

        Dim mEmpresa As New ClsEmpresas
        mEmpresa = ClsEmpresas.GetEmpresasByCod("1")
        If mAno = mEmpresa.Ano_Trabalho Then
            MsgBox("Ano atual já atualizado")
            Exit Sub
        End If
        Dim mListaTD As New List(Of ClsTipos_Doc)

        mListaTD = ClsTipos_Doc.GetTipos_Doc

        For Each mTD As ClsTipos_Doc In mListaTD
            Dim mSerie As New ClsSeries
            mSerie = New ClsSeries(mAno & "-" & mTD.FLG_Tipo_Doc, mTD.FLG_Tipo_Doc, mAno)
            If Not GravaSeries(mSerie) Then
                MsgBox("Erro na inicialização da Série do Tipo de Documento " & mTD.FLG_Tipo_Doc)
            End If

            '
            ' Update do Tipos_Doc
            '
            mTD.Cod_Serie = mSerie.Cod_Serie
            If Not ClsTipos_Doc.ModificaTipos_Doc(mTD) Then
                MsgBox("Erro na inicialização do Tipo de Documento " & mTD.FLG_Tipo_Doc)
            End If
        Next

        mEmpresa.Ano_Trabalho = mAno
        If Not ClsEmpresas.ModificaEmpresas(mEmpresa) Then
            MsgBox("Erro na inicialização Ano de Trabalho da Empresa ")
        End If
